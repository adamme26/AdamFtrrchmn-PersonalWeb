...
<body>
  ...
  <header class="bg-[#4e342e] text-white text-center py-10 shadow-md">
    <h1 class="text-3xl md:text-4xl font-bold">Tambah Data About</h1>
    <p class="text-sm italic mt-2 text-[#ffe0b2]">Catat kisah dan perjalananmu di sini.</p>
  </header>
  ...
  <div class="flex flex-col lg:flex-row max-w-7xl mx-auto mt-10 px-6 gap-6">
    ...
    <aside class="lg:w-1/4 card rounded-xl p-6">
      ...
      <ul class="space-y-4 text-base">
        ...
        <li><a href="data_gallery.php" class="block px-3 py-2 rounded nav-link">Kelola Gallery</a></li>
        <li><a href="about.php" class="block px-3 py-2 rounded bg-[#d7ccc8] font-semibold text-[#3e2723]">About</a></li>
    ...
    <main class="lg:w-3/4 card rounded-xl p-8">
      <form action="proses_add_about.php" method="post" class="space-y-6">
        <div>
          <label for="about" class="block text-sm font-medium text-[#5d4037] mb-1">Isi Tentang Saya</label>
          <textarea id="about" name="about" rows="6" required
            class="w-full p-3 border border-[#bcaaa4] rounded focus:outline-none focus:ring focus:border-[#6d4c41] bg-white text-[#3e2723]"></textarea>
        </div>

        <div class="flex justify-end space-x-4">
          <button type="submit" class="bg-[#6d4c41] text-white px-4 py-2 rounded hover:bg-[#4e342e] transition">Simpan</button>
          <a href="about.php" class="bg-gray-300 text-[#3e2723] px-4 py-2 rounded hover:bg-gray-400 transition">Batal</a>
        </div>
      </form>
    </main>
